// =======================================================================
// e-Rapor Quantum — AUTH SYSTEM (Super Sinkron dengan KODE.GS & Dashboard)
// =======================================================================

const WEBAPP_URL =
  "https://script.google.com/macros/s/AKfycbzSNseVpWuNseMTG0ilYHXb8y8xndFDe4DjS7BLFB36OsC3WHZ6YSYv6n4fjkhYTfesrA/exec";
...
async function loginUser(forceUsername, forcePassword) {
  const username =
    forceUsername || document.getElementById("usernameLogin")?.value?.trim();
  const password =
    forcePassword || document.getElementById("passwordLogin")?.value?.trim();
  ...
  try {
    ...
    if (data.success) {
      // Simpan semua data dalam satu objek (sinkron dengan dashboard.js)
      const session = {
        token_sesi: data.token_sesi,
        username: data.username,
        nama_lengkap: data.nama_lengkap,
        token_unik_sekolah: data.token_unik_sekolah || data.token_unik || "",
        spreadsheet_id_sekolah: data.spreadsheet_id_sekolah || "",
        login_expire: Date.now() + 3600000, // 1 jam
      };
      localStorage.setItem("erapor_session", JSON.stringify(session));
