/* auth.js — client code untuk GitHub Pages (fetch ke GAS) */
/* GANTI nilai ini dengan URL Web App (Deploy as web app) */
const GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbz1W1BESRCIjZbzFtvDmVctegROoAb1B7X9VWL5wH8bxbChs0KBvT4elzoF5u2m3Wkk/exec";
...
async function callGAS(action, payload) {
  try {
    ...
    // jika server mengembalikan non-JSON atau error, tangani
...
async function validateToken() {
  ...
  if (response.success) {
    ...
    if (response.needsRegister) {
      Swal.fire({ icon:"success", title:"Token Diterima!", text:"Mengalihkan ke registrasi...", timer:1100, showConfirmButton:false });
      setTimeout(() => _showSection("registerSection"), 1100);
    } else {
      Swal.fire({ icon:"info", title:"Token Dikenali", text:"Silakan login.", showConfirmButton:true });
...
async function registerUser() {
  ...
  if (response && response.success) {
    Swal.fire({ icon:"success", title:"Registrasi Berhasil", text:"Masuk ke dashboard...", showConfirmButton:false, timer:1200 });
...
async function loginUser(forceUsername, forcePassword) {
  ...
  if (response && response.success) {
    ...
    Swal.fire({ icon:"success", title:"Login Berhasil", text:"Mengalihkan ke dashboard...", showConfirmButton:false, timer:1200 });
...
function lupaPassword() {
  Swal.fire({
    ...
    if (res.isConfirmed && res.value) {
      Swal.fire({ title: "Mengubah password...", text: "Mohon tunggu sebentar", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
      ...
      Swal.fire({
        ...
        text: response && (response.message || response.error) || "Terjadi kesalahan"
      });
    }
  });
}
